const baseUrl = process.env.SMOKE_BASE_URL || 'http://127.0.0.1:5214';
...
const adminCookie = await login('admin@ai-local-reputation.local', 'admin123');
const managerCookie = await login('manager@ai-local-reputation.local', 'manager123');
const analystCookie = await login('analyst@ai-local-reputation.local', 'analyst123');

await expectJson('/api/dashboard', adminCookie);
await expectJson('/api/entities/review-requests', analystCookie);
...
const records = await expectJson('/api/entities/review-requests', managerCookie);
const rowId = records.rows[0].id;

const approveResponse = await fetch(`${baseUrl}/api/entities/review-requests/approve`, {
  method: 'POST',
  headers: {
...
const forbiddenApprove = await fetch(`${baseUrl}/api/entities/review-requests/approve`, {
  method: 'POST',
  headers: {
...
console.log('AI Local Business Reputation Suite smoke passed');
